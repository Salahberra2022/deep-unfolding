...
class SOR(nn.Module):
    ...
    def forward(self, num_itr, bs,y):
        traj = []
        omega = torch.tensor(1.5);     inv_omega = torch.div(1,omega);
...
class AOR(nn.Module):
    ...
    def forward(self, num_itr, bs,y):
        traj = []
        r=torch.tensor(0.1)        # accleration parameter
        omega = torch.tensor(0.2)  #overelexation parameter  
...
Iterative = ['GS', 'RI', 'Jacobi','SOR', 'ChebySOR', 'AOR', 'ChebyAOR']

#SER     = np.zeros((len(DETECTOR), len(snr_range), test_steps)) 
#norm_list = np.zeros(len(DETECTOR))
norm_list = np.zeros((len(Iterative), len(itr_list)))
...
for k,iterative in enumerate(Iterative):
    if iterative == 'GS': 
